...
class Connection {
    ...
    updateProtobufData(resolve, handler) {
        ...
        return new Promise((res, rej) => {
            ...
            const attempt = (func, errorText) => {
                try {
                    func();
                } catch(error) {
                    this.debug(errorText, error);
                }
            };
            ...
            this.pingTimer = setInterval(() => {
                try {
                    client.ping(Buffer.from(uuidv4().substr(0, 8)), (err, duration, payload) => {
                        if (!err) {
                            this.verbose('API observe PING:', duration, 'ms, payload', '\'' + payload.toString() + '\'');
                            if (this.timeoutTimer) {
                                clearTimeout(this.timeoutTimer);
                                this.timeoutTimer = setTimeout(timeoutFunction, API_OBSERVE_TIMEOUT_SECONDS * 1000);
                            }
                        } else {
                            this.verbose('API observe PING: response error', err.code || err);
                        }
                    });
                } catch(error) {
            ...
            req.on('data', data => {
                ...
                // Always buffer first — HTTP/2 chunk boundaries are arbitrary, so a chunk
                // may carry a partial header, a partial body, one message, or several.
                // Decoding the length prefix before buffering throws RangeError when the
                // varint is split across chunks and crashes the child bridge (#677/#742).
                protoBuffer = Buffer.concat([protoBuffer, data]);

                // Drain every complete length-delimited message currently buffered.
                // Frame layout: [1 prefix byte][varint body-length][body].
                while (true) {
                    if (pendingLength == 0) {
                        if (protoBuffer.length < 2) {
                            break;
                        }
                        let bodyLength;
                        try {
                            bodyLength = varint.decode(protoBuffer, 1);
                        } catch (error) {
                            // Incomplete length prefix — wait for the next chunk.
                            // A permanently corrupt stream will time out via timeoutTimer.
                            break;
                        }
                        if (!varint.decode.bytes) {
                            break;
                        }
                        pendingLength = bodyLength + varint.decode.bytes + 1;
                    }
                    ...
                    pendingLength = 0;
                    this.verbose('API observe POST: complete message received (' + protoMessage.length + ')');
    ...
    protobufToNestLegacy(protobuf) {
        ...
        const initDevice = (deviceType, deviceId, structureId, fwVersion, body, protobufDeviceType) => {
            ...
            if (object && object.message && object.message.length > 0) {
                ...
                if (object) {
                    ...
                    translateProperty(object, 'peer_devices', null, (peerDevice, id) => {
                        ...
                        if (!peerDevice.object.id.startsWith('STRUCTURE_')) {
                            ...
                            peerDevice.data.property.value.devices.forEach(el => {
                                ...
                                if (body.track && body.track[deviceId] && !body.track[deviceId].online) {
                                ...
                                } else if (['nest.resource.NestLearningThermostat3Resource', 'nest.resource.NestAgateDisplayResource', 'nest.resource.NestOnyxResource', 'google.resource.GoogleZirconium1Resource', 'nest.resource.NestLearningThermostat3v2Resource', 'nest.resource.NestThermostat3Resource', 'nest.resource.NestAmber2DisplayResource'].includes(deviceType)) {
                                    ...
                                    initDevice('device', deviceId, structureId, el.data.fwVersion, body, deviceType);
                                    ...
                                    initDevice('kryptonite', deviceId, structureId, el.data.fwVersion, body, deviceType);
                                    ...
                                    this.log('UNSUPPORTED DEVICE TYPE DETECTED: ' + deviceType + ' (Device ID: ' + deviceId + ') - Please report this');
                    ...
                    translateProperty(object, 'fan_control', null, (fanControl, id) => {
                        ...
                        if (checkDeviceExists(body, id)) {
                            body.device[id].hvac_fan_state = ['FAN_SPEED_SETTING_STAGE1', 'FAN_SPEED_SETTING_STAGE2', 'FAN_SPEED_SETTING_STAGE3'].includes(fanControl.data.property.value.currentSpeed);
                        }
                    });
                    ...
                    translateProperty(object, 'fan_control_settings', null, (fanControlSetting, id) => {
                        if (checkDeviceExists(body, id)) {
                            ...
                            body.device[id].fan_timer_duration = Math.max(body.device[id].fan_timer_active ? fanControlSetting.data.property.value.fanTimerTimeout.value - (Date.now() / 1000) : 0, 0);
                    ...
                    translateProperty(object, 'structure_mode', null, (structureMode, id) => {
                    ...
                    translateProperty(object, 'battery_power_source', null, (batteryStatus, id) => {
                        let deviceKey = this.legacyDeviceMap[id];
                        if (checkDeviceExists(body, id)) {
    ...
    apiResponseToObjectTree(body) {
        ...
        for (const structureId in structures) {
            ...
            data.devices['home_away_sensors'][structureId].away = thisStructure.new_structure_id ? thisStructure.protobuf_away : thisStructure.away;
            ...
            swarm.map(unit => unit.split('.')).forEach(unit => {
                try {
                    ...
                    if (deviceType == 'device') {
                        ...
                        if (thisDevice.has_eco_mode && thisDevice.protobuf_device_type != 'google.resource.GoogleZirconium1Resource') {
            ...
            try {
                ...
                if (!['ESOCKETTIMEDOUT', 'ECONNABORTED'].includes(error.code)) {
                    ...
                    if (error.status == 401 || error.status == 403 || ['ECONNREFUSED', 'ENETUNREACH'].includes(error.code)) {
                        ...
                        await Promise.delay(API_RETRY_DELAY_SECONDS * 1000);
                    }
                }
            }
    ...
    async protobufDataTimerLoop(resolve, handler) {
        while (true) {
            this.verbose('API observe POST: streaming request initiated');
            try {
                await this.updateProtobufData(resolve, handler);
    ...
    legacyToProtobufUpdate(body) {
        function addToOutput(output, currentState, device, protoKey, protoType, key, value) {
            ...
            if (key == 'target_temperature_type') {
                ...
                found.property.value.timerDuration = { value: value ? (this.config.fanDurationMinutes || DEFAULT_FAN_DURATION_MINUTES) * 60 : 0 };
                ...
                found.property.value.enabled = 1;
                found.property.value.units = value == 'F' ? 'DEGREES_F' : 'DEGREES_C';
        ...
        data.forEach(req => {
            ...
            for (const key in prop) {
                ...
                } else if (['temperature_scale'].includes(key)) {
                    protoKey = 'display_settings';
                    protoType = 'type.nestlabs.com/nest.trait.hvac.DisplaySettingsTrait';
